...
export default function PatientProgress({ session }) {
  ...
  if (!data) return <p style={{ color: "var(--text-muted)", fontSize: 14 }}>Carregando...</p>;
  ...
  return (
    <div style={{ animation: "fadeUp .4s ease" }}>
      ...
      {data.scalePts.length < 2 && data.moodPts.length < 2 && (
        <div className="card">
          <EmptyState icon="📈" message="Complete mais exercícios e registros no diário para ver seu progresso aqui." />
        </div>
      )}
    </div>
  );
