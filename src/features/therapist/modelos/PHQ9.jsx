...
const QUESTIONS = [
  ...
  "Sentir-se mal consigo mesmo(a), achar que é um fracasso",
  "Dificuldade de concentração (ler, assistir TV, etc.)",
  "Mover-se ou falar tão devagar que outros perceberam, ou o oposto",
...
const SEVERITY = [
  ...
  { min: 15, max: 19, label: "Moderadamente Grave", color: "#e07b7b", bg: "#fdeaea" },
  { min: 20, max: 27, label: "Grave", color: "#c0392b", bg: "#fde0de" },
];
...
export default function PHQ9({ patientName = "Paciente" }) {
  ...
  const handleReset = () => { setAnswers(Array(9).fill(null)); setSubmitted(false); };
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.text("Escala PHQ-9", pageW / 2, 13, { align: "center" });
      ...
      pdf.text(`Pontuação Total: ${total} / 27`, 14 + 8, y + 9);
      ...
      pdf.save(`phq9_${patientName.replace(/\s+/g, "_")}_${today.replace(/\//g, "-")}.pdf`);
  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.topBar}>
        <div>
          <h2 className={styles.title}>😟 Escala PHQ-9</h2>
          <p className={styles.subtitle}>Triagem padronizada para depressão — últimas 2 semanas</p>
        </div>
        ...
        {submitted && (
          <div className={styles.resultCard} style={{ borderColor: severity.color, background: severity.bg }}>
            ...
            <div className={styles.resultInfo}>
              ...
              <span className={styles.resultRange}>Pontuação de {total}/27 pontos</span>
            </div>
