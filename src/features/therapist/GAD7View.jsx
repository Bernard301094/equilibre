...
const QUESTIONS = [
  ...
  "Irritar-se ou aborrecer-se facilmente",
  "Sentir medo como se algo terrível fosse acontecer",
];

function getSeverity(score) {
  if (score <= 4) return { label: "Mínima", color: "#22c55e" };
  if (score <= 9) return { label: "Leve", color: "#84cc16" };
  if (score <= 14) return { label: "Moderada", color: "#f59e0b" };
  return { label: "Grave", color: "#ef4444" };
}

export default function GAD7View({ patientName = "Paciente" }) {
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "24px 16px", fontFamily: "inherit" }}>
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ fontSize: 24, fontWeight: 700, marginBottom: 4 }}>😰 GAD-7 — Escala de Ansiedade</h2>
              ...
              {OPTIONS.map((option) => (
                <label
                  ...
                  style={{
                    ...
                    padding: "6px 14px",
                    borderRadius: 8,
                    border: answers[index] === option.value ? "2px solid #8b5cf6" : "1px solid #d1d5db",
                    background: answers[index] === option.value ? "#f5f3ff" : "#f9fafb",
                    cursor: "pointer",
                    fontSize: 14,
                  }}
                >
