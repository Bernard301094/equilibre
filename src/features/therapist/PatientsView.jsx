...
export default function PatientsView({ session }) {
  ...
  if (loading) {
    return (
      <div className="patients-view page-fade-in">
        <div className="patients-view__header">
          <h2 className="patients-view__title">Meus Pacientes</h2>
          <p className="patients-view__subtitle">Gerencie seus pacientes e envie convites</p>
        </div>
        <div className="patients-view__grid">
          <div className="patients-view__card"><SkeletonList rows={3} /></div>
  ...
  return (
    <div className="patients-view page-fade-in">
      ...
      <div className="patients-view__grid">
        ...
        {/* ── Pacientes ativos ── */}
        <div className="patients-view__card">
          <h3 className="patients-view__section-title">
          ...
          {patients.length === 0 ? (
            ...
            <div className="patients-view__list">
              {patients.map((p) => {
                ...
                return (
                  <div key={p.id} className="patient-row">
                    ...
                    <div className="patient-row__info">
                      ...
                      />
                      <div className="patient-row__text">
                        <div className="patient-row__name">{p.name}</div>
                        <div className="patient-row__email">{p.email}</div>
                      </div>
                    </div>

                    {/* Streak pill */}
                    <div
                      className={["patient-row__streak", streak === 0 ? "patient-row__streak--zero" : ""].filter(Boolean).join(" ")}
                      title={`${stage.label} — ${streak} dia${streak !== 1 ? "s" : ""} seguido${streak !== 1 ? "s" : ""}`}
                    >
                      <span className="patient-row__streak-icon" aria-hidden="true">{stage.icon}</span>
                      <span
                        className="patient-row__streak-val"
                    ...
                    <div className="patient-row__actions">
                      <button
                        className="patient-row__btn patient-row__btn--manage"
                        onClick={() => setManagingPatient(p)}
                      >
                        Gerenciar
                      </button>
                      <button
                        className="patient-row__btn patient-row__btn--unlink"
        ...
        {/* ── Convites ── */}
        <div className="patients-view__card">
          <h3 className="patients-view__section-title">Convidar Novo Paciente</h3>
          <p className="patients-view__invite-desc">
            Gere um código único e partilhe com o seu paciente. Ao usá-lo no registo,
            ele ficará automaticamente vinculado à sua conta.
          </p>

          <button
            className="patients-view__gen-btn"
          ...
          <h4 className="patients-view__invites-label">Convites Gerados</h4>

          {invites.length === 0 && (
            <p className="patients-view__invites-empty">Nenhum convite gerado ainda.</p>
          )}

          <div className="patients-view__invites-list">
            {invites.map((inv) => (
              <div
                key={inv.code}
              ...
              >
                ...
                {inv.status === "used" ? (
                  <div className="invite-row__used">
                    <span className="invite-row__code invite-row__code--used">{inv.code}</span>
                    <span className="invite-row__badge-used">Utilizado</span>
                  </div>
                ) : (
                  <div className="invite-row__actions">
                    <span className="invite-row__code">{inv.code}</span>

                    <button
                      className={["invite-row__action-btn", copiedCode === inv.code ? "invite-row__action-btn--copied" : ""].filter(Boolean).join(" ")}
                    ...
                    >
                      {copiedCode === inv.code ? (
                        <>
                          ...
                          <span className="invite-row__copied-label">Copiado</span>
                        </>
                      ) : (
                        <span aria-hidden="true">📋</span>
                      )}
                    ...
                    <button
                      className="invite-row__action-btn"
                    ...
                    <button
                      className="invite-row__action-btn invite-row__action-btn--delete"
  ...
  return (
    <div className="confirm-overlay" onClick={onClose}>
      ...
      >
        ...
        <div id={titleId} className="confirm-modal__title">{title}</div>
        <div className="confirm-modal__desc">{children}</div>
